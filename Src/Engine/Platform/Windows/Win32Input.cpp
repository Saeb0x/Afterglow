...
void Win32InputProcess(HWND windowHandle, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch(message)
    {
        ...
        case WM_MBUTTONUP:
        {
            ...
            // NOTE(saeb): Capture while any button is held so the button-up still arrives outside the window. State is updated first because ReleaseCapture() sends WM_CAPTURECHANGED synchronously. SetCapture() sends WM_CAPTURECHANGED even when this window already has capture, so only take it once.
            if(isDown)
            {
